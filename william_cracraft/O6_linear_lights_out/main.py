...
time_text = play.new_text(f"Timer = 0",
                          y=-225, color='lime')
...
play.new_text("Linear Lights Out",
              ...
              font_size=100,
              color='lime')
score_text = play.new_text(f"You Won in _ moves!", y=100, font_size=50, color='lime')
score_text.hide()

win_light = play.new_image("light_on.png", y=-150, size=75)
...
async def control_lights():
    if win_light.game_over:
        ...
        score_text.words = f"You won in {win_light.counter} moves and in {time_text.words}"
        if win_light.counter == 1:
            score_text.words = f"You won in {win_light.counter}"
        score_text.show()
        win_light.show()
        return

    time_text.words = f"{(time.time() - start_time):.2f} seconds"
